...
static int pusherror(lua_State *L, const char *info)
{
	lua_pushnil(L);
	if (info == NULL)
		lua_pushstring(L, strerror_s(buff, sizeof(buff), errno));
	else
		lua_pushfstring(L, "%s: %s", info, strerror_s(buff, sizeof(buff), errno));
...
static int change_dir(lua_State *L) {
	...
	if (chdir(path)) {
		...
		lua_pushfstring(L, "Unable to change working directory to '%s'\n%s\n",
			path, strerror_s(buff, sizeof(buff), errno));
		return 2;
	}
	else {
		lua_pushboolean(L, 1);
...
static int lfs_lock_dir(lua_State *L) {
	...
	if (!ln) {
		lua_pushnil(L); lua_pushstring(L, strerror_s(buff, sizeof(buff), errno)); return 2;
	}
	strcpy_s(ln, sizeof(ln), path); strcat_s(ln, sizeof(ln), lockfile);
	if ((fd = CreateFile(ln, GENERIC_WRITE, 0, NULL, CREATE_NEW,
		...
		if (en == ERROR_FILE_EXISTS || en == ERROR_SHARING_VIOLATION)
			...
			lua_pushstring(L, strerror_s(buff, 100, en));
		return 2;
	}
	free(ln);
...
static int lfs_lock_dir(lua_State *L) {
	...
	if (symlink("lock", ln) == -1) {
		free(ln); lua_pushnil(L);
		lua_pushstring(L, strerror_s(buff, sizeof(buff), errno)); return 2;
	}
	lock->ln = ln;
	luaL_getmetatable(L, LOCK_METATABLE);
	lua_setmetatable(L, -2);
...
static int file_lock(lua_State *L) {
	...
	if (_file_lock(L, fh, mode, start, len, "lock")) {
		...
		lua_pushnil(L);
		lua_pushfstring(L, "%s", strerror_s(buff, sizeof(buff), errno));
		return 2;
	}
}
...
static int dir_iter_factory(lua_State *L) {
	...
	d->dir = opendir(path);
	if (d->dir == NULL)
		luaL_error(L, "cannot open %s: %s", path, strerror_s(buff, sizeof(buff), errno));
#endif
	return 2;
}
...
static int _file_info_(lua_State *L, int(*st)(const char*, STAT_STRUCT*)) {
	...
	if (st(file, &info)) {
		lua_pushnil(L);
		lua_pushfstring(L, "cannot obtain information from file '%s': %s", file, strerror_s(buff, sizeof(buff), errno));
		return 2;
	}
	if (lua_isstring(L, 2)) {
