...
namespace riko::image {
    ...
    static int imageCopy(lua_State *L) {
        ...
        int result = SDL_BlitSurface(src->surface, &srcRect, dst->surface, &rect);

        for (int xp = 0; xp < srcRect.w; xp++) {
            for (int yp = 0; yp < srcRect.h; yp++) {
                int xp2 = xp + srcRect.x;
                int yp2 = yp + srcRect.y;
                if (xp2 >= src->width || xp2 < 0 || yp2 >= src->height || yp2 < 0) {
                    continue;
                }
                char c = src->internalRep[xp2][yp2];
                if (c >= 0) {
                    internalDrawPixel(dst, x + xp, y + yp, c);
                }
            }
        }
